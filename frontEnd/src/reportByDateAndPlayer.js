...
const getPlayersReport = async () => {
    ...
    const response = await getDailyReportByDateAndPlayer.get()

    // ✅ Backend nuevo: { porDia: [], totales: {} }
    const reports = response.porDia;
    const totales = response.totales;

    if (reports.length > 0) {
        console.log(reports);
        requestReport(reports)

        // ✅ TOTAL con clase diferente
        if (totales) {
            console.log(totales)
            requestTotal(totales)
        }
...
// ✅ TOTAL separado, simple y con clase diferente
const requestTotal = (total) => {
    const row = document.createElement("div");
    row.classList.add("clientAndDate-reports__dynamic");
    row.classList.add("clientAndDate-reports__dynamic-total");

    const totalElement = document.createElement("p");
    totalElement.classList.add("clientAndDate-report__dynamic__text")
    totalElement.textContent = "TOTAL";
    row.appendChild(totalElement);

    const cashElement = document.createElement("p");
    cashElement.classList.add("clientAndDate-report__dynamic__text");
    cashElement.textContent = formatearMoneda(total.totalCash)
    row.appendChild(cashElement);

    const creditElement = document.createElement("p");
    creditElement.classList.add("clientAndDate-report__dynamic__text");
    creditElement.textContent = formatearMoneda(total.totalCredit);
    row.appendChild(creditElement);

    const dollarElement = document.createElement("p");
    dollarElement.classList.add("clientAndDate-report__dynamic__text");
    dollarElement.textContent = formatearMoneda(total.totalDollar)
    row.appendChild(dollarElement);

    const paymentElement = document.createElement("p");
    paymentElement.classList.add("clientAndDate-report__dynamic__text");
    paymentElement.textContent = formatearMoneda(total.totalPayment)
    row.appendChild(paymentElement);

    const netwinElement = document.createElement("p");
    netwinElement.classList.add("clientAndDate-report__dynamic__text");
    netwinElement.textContent = formatearMoneda(total.netwin);
    row.appendChild(netwinElement)
