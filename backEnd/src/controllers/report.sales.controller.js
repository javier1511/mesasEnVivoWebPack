  ...
  try {
    ...
    const [data] = await Sales.aggregate([
...

      {
        $project: {
          porDia: 1,
          totales: { $ifNull: [{ $arrayElemAt: ["$totales", 0] }, null] },
        },
      },
    ]);

    // Siempre regresamos el mismo formato
    if (!data || !data.porDia || data.porDia.length === 0) {
      return res.json({ porDia: [], totales: null });
    }
